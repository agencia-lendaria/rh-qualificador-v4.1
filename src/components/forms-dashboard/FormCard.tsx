...
export function FormCard({ form, onFormClick, onExport, onDelete }: FormCardProps) {
  ...
  return (
    <div className="modern-card cursor-pointer group animate-fade-in hover:scale-102 transition-all duration-300 flex flex-col h-full" onClick={handleCardClick}>
      {/* Header */}
      <div className="flex items-start justify-between mb-4 flex-shrink-0">
        <div className="flex-1 min-w-0 pr-2">
          <div className="flex items-center space-x-2 mb-2">
            <div className="w-7 h-7 bg-gradient-to-br from-primary to-primary-dark rounded-lg flex items-center justify-center flex-shrink-0">
              <FileText className="w-3 h-3 text-white" />
            </div>
            <div className="text-xs text-text-muted font-medium uppercase tracking-wider truncate">Form #{form.id}</div>
          </div>
          <h3 className="text-sm lg:text-base font-bold text-text-primary mb-2 group-hover:text-primary transition-colors line-clamp-2 leading-tight">{form.vaga_do_form}</h3>
          <div className="flex items-center space-x-2 text-xs text-text-secondary">
            <Calendar className="w-3 h-3 flex-shrink-0" />
            <span className="truncate">{form.created_at ? new Date(form.created_at).toLocaleDateString('pt-BR', { day: '2-digit', month: 'short' }) : 'N/A'}</span>
          </div>
        </div>
        <div className={`status-badge ${statusColor} text-xs flex-shrink-0`}>
          {form.status === 'active' ? (
            <>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4 flex-shrink-0">
        <div className="text-center">
          <div className="text-xl font-bold text-primary mb-1">{form.candidateCount}</div>
          <div className="text-xs text-text-secondary uppercase tracking-wide">Candidatos</div>
        </div>
        <div className="text-center">
          <div className="text-xl font-bold text-secondary mb-1">{form.questionCount}</div>
      ...
      {/* Score */}
      {form.averageScore && (
        <div className="mb-4 p-2 bg-success/10 rounded-lg border border-success/20 flex-shrink-0">
      ...
      <div className="pt-3 border-t border-border space-y-2 flex-shrink-0">
        ...
        <div className="flex items-center gap-2">
          <Button
            variant="outline"
            className="btn-modern btn-primary shimmer-button text-xs px-2 lg:px-3 py-2 font-medium flex-1"
          ...
          <Button
            variant="outline"
            className="btn-modern shimmer-button text-xs px-2 py-2"
        ...
        {form.candidateCount > 0 && (
          <div className="flex">
            <Button
              variant="outline"
              className="btn-modern btn-success shimmer-button text-xs px-2 lg:px-3 py-2 font-medium w-full"
